...
集中配置中心，把所有服务的配置文件都放在一个代码管理平台上（github，码云），通过该服务去读取对应服务所需要的配置文件；这样的话，项目发布之后，可以在管理平台上修改配置文件，从而不用影响项目的运行。
...
获取jenkins密码：cat /var/lib/jenkins/secrets/initialAdminPassword
